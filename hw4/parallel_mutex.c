...
// Structure representing a bucket entry in the hash table
...
pthread_mutex_t bucket_mutexes[NUM_BUCKETS]; // Array of mutexes, one for each bucket
...
void insert(int key, int val) {
  ...
  // Unlock the mutex for the bucket
...
bucket_entry * retrieve(int key) {
  ...
  for (b = table[i]; b != NULL; b = b->next) {
    if (b->key == key) {
      pthread_mutex_unlock(&bucket_mutexes[i]); // Unlock and return the entry if found
      return b;
    }
  }
